...
parser.add_argument("-d", "--max-depth", help='max per-file depth', type=int, default=2500)
parser.add_argument("-f", "--fasta-ref", help='faidx indexed reference sequence file (needed for CRAM input files)')
    ...
    if nbamrc >= args.min_RC:
        ...
        if bamchr == snpchr and bampos == snppos:
            ...
            while isamp < nsamples:
                ...
                rcounts[irc] += ibases.count('.')    # ref allele when using -f
                rcounts[irc] += ibases.count(',')    # ref allele when using -f
                irc += 1
